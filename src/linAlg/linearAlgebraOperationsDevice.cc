...
namespace dftfe
{
  namespace linearAlgebraOperationsDevice
  {
      ...
      const dftParameters &                        dftParams)
    {
      ...
      dftfe::utils::deviceMemcpyD2D(
        dftfe::utils::makeDataTypeDeviceCompatible(XArrayCM.begin()),
      ...
      dftfe::utils::deviceKernelsGeneric::axpby(
        totalVectorSize, XArrayCM.begin(), YArrayCM.begin(), alpha2, (double)1);
          ...
          if (degree == 2)
          ...
          else
            {
              ...
              combinedDeviceKernel<<<
                ...
                numberVectors,
                localVectorSize,
                dftfe::utils::makeDataTypeDeviceCompatible(YArrayCM.begin()),
                dftfe::utils::makeDataTypeDeviceCompatible(XArrayCM.begin()),
                coeff,
                alpha2,
                alpha1,
  ...
  pcout << "Chebyshev Mean Time: " << chebyMean << "\n"
    ...
    namespace
    {
      ...
      __global__ void
      scaleKernel(const ValueType *__restrict__ X,
                  ValueType *__restrict__ Y,
                  const ValueType        a,
                  const ValueType        b,
                  const dftfe::size_type N)
      {
        for (dftfe::size_type i = threadIdx.x + blockIdx.x * blockDim.x; i < N;
             i += blockDim.x * gridDim.x)
          Y[i] = a * X[i] + b * Y[i];
      }
      ...
      template <typename ValueType>
      __global__ void
      blockScaleKernel(ValueType *__restrict__ x,
                       const ValueType *__restrict__ s,
                       const ValueType        a,
      ...
      __global__ void
      combinedDeviceKernelMF(double *__restrict__ x,
                             double *__restrict__ y,
                             const double a,
                             const double b,
                             const double scalar,
                             const double scalarOld,
                             const double *__restrict__ invSqrtMassVec,
                             const double *__restrict__ sqrtMassVec,
                             const dftfe::size_type nDofs)
      {
        int dofIdx = threadIdx.y + blockIdx.x * blockDim.y;
        int idx =
        ...
        if (dofIdx < nDofs)
          {
            double X = x[idx];
            double Y = y[idx];
            Y        = Y * sqrtMassVec[dofIdx] / scalarOld;
            X        = X * invSqrtMassVec[dofIdx];
            Y        = a * X + b * Y;
            x[idx]   = X * invSqrtMassVec[dofIdx] * scalar;
            y[idx]   = Y * sqrtMassVec[dofIdx]; //*/
      ...
      __global__ void
      reshapeKernel(double *__restrict__ y,
                    double *__restrict__ x,
                    const dftfe::size_type nDofs,
                    bool                   CVtoBCVFlag)
      {
        dftfe::size_type dofIdx = threadIdx.y + blockIdx.x * blockDim.y;

        dftfe::size_type i =
          threadIdx.x + dofIdx * blockDim.x + blockIdx.y * blockDim.x * nDofs;

        dftfe::size_type j = threadIdx.x + blockIdx.y * blockDim.x +
                             dofIdx * blockDim.x * gridDim.y;
      ...
      const dftParameters &                        dftParams)
    {
      ...
      dftfe::linearAlgebra::createMultiVectorFromDealiiPartitioner(
        operatorMatrix.getMFBatchedPartitioner(), 1, YArrayMF); //*/

      const unsigned int totalVectorSize = localVectorSize * numberVectors;

      constexpr int batchSize = 4;
      const int     batch     = numberVectors / batchSize;

      dim3 threads(batchSize, dftfe::utils::DEVICE_BLOCK_SIZE / batchSize);

      dim3 blocks((batchSize * localVectorSize +
                   dftfe::utils::DEVICE_BLOCK_SIZE - 1) /
                    dftfe::utils::DEVICE_BLOCK_SIZE,
                  batch);

      reshapeKernel<<<blocks, threads>>>(XArrayMF.begin(),
                                         XArray.begin(),
                                         localVectorSize,
                                         true);
      ...
      // YArray = YArray + alpha2*XArray and YArray = alpha1 *YArray
      //
      scaleKernel<<<(totalVectorSize + dftfe::utils::DEVICE_BLOCK_SIZE - 1) /
                      dftfe::utils::DEVICE_BLOCK_SIZE,
                    dftfe::utils::DEVICE_BLOCK_SIZE>>>(
        dftfe::utils::makeDataTypeDeviceCompatible(XArrayMF.begin()),
        dftfe::utils::makeDataTypeDeviceCompatible(YArrayMF.begin()),
        dftfe::utils::makeDataTypeDeviceCompatible(alpha1 * alpha2),
        dftfe::utils::makeDataTypeDeviceCompatible(alpha1),
        totalVectorSize);
      ...
      for (unsigned int degree = 2; degree < m + 1; ++degree)
        {
          ...
          if (degree == 2)
            {
              scaleKernel<<<(totalVectorSize + dftfe::utils::DEVICE_BLOCK_SIZE -
                             1) /
                              dftfe::utils::DEVICE_BLOCK_SIZE,
                            dftfe::utils::DEVICE_BLOCK_SIZE>>>(
                dftfe::utils::makeDataTypeDeviceCompatible(YArrayMF.begin()),
                dftfe::utils::makeDataTypeDeviceCompatible(XArrayMF.begin()),
                dftfe::utils::makeDataTypeDeviceCompatible(coeff),
                dftfe::utils::makeDataTypeDeviceCompatible(alpha2),
                totalVectorSize);

              XArrayMF.zeroOutGhosts();
          ...
          else if (degree == m)
            {
              ...
              blockScaleKernel<<<blocks, threads>>>(
                ...
                dftfe::utils::makeDataTypeDeviceCompatible(
                  operatorMatrix.getSqrtMassVec()),
                dftfe::utils::makeDataTypeDeviceCompatible(1.0 / alpha1Old),
                dftfe::utils::makeDataTypeDeviceCompatible(localVectorSize));
            ...
            {
              combinedDeviceKernelMF<<<blocks, threads>>>(
                dftfe::utils::makeDataTypeDeviceCompatible(YArrayMF.begin()),
                dftfe::utils::makeDataTypeDeviceCompatible(XArrayMF.begin()),
                coeff,
                alpha2,
