...
namespace dftfe
{
  ...
  {
    /**
     * Internal function to determine dimension of process grid based on the
     * total number of cores, matrix dimensions and matrix block sizes
     *
     * Amesos heuristics:
                                 ...
                                 const unsigned int block_size_n)
    {
      ...
      // be done. LU , QR and QL factorizations perform better for “flat”
      // process grids (Pr < Pc ) For large N, Pc = 2*Pr is a good choice,
      // whereas for small N, one should choose small Pr Square or near square
      // grids are more optimal for Cholesky factorization. LQ and RQ
    ...
    if (mpi_communicator_inactive_with_root != MPI_COMM_NULL)
      {
        const int ierr =
          MPI_Bcast(value,
                    count,
                    dealii::Utilities::MPI::internal::MPIDataTypes::mpi_type_id(
                      value),
                    0 /*from root*/,
                    mpi_communicator_inactive_with_root);
        AssertThrowMPI(ierr);
...
template void
dftfe::ProcessGrid::send_to_inactive<double>(double *, const int) const;
template void
dftfe::ProcessGrid::send_to_inactive<float>(float *, const int) const;
template void
dftfe::ProcessGrid::send_to_inactive<int>(int *, const int) const;
template void
dftfe::ProcessGrid::send_to_inactive<std::complex<double>>(
  std::complex<double> *,
  const int) const;
