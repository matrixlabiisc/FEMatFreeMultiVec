...
namespace dftfe
{
  ...
  {
    __global__ void
    copyFloatArrToDoubleArrLocallyOwnedMF(
      const float *       floatArr,
      const unsigned int *locallyOwnedFlagArr,
      double *            doubleArr,
      const unsigned int  nDofs)
    {
      int dofIdx = threadIdx.y + blockIdx.x * blockDim.y;
      int idx =
        threadIdx.x + dofIdx * blockDim.x + blockIdx.y * blockDim.x * nDofs;

      if ((dofIdx < nDofs) and (locallyOwnedFlagArr[dofIdx] == 1))
        doubleArr[idx] = floatArr[idx];
    }
  ...
  kohnShamDFTOperatorDeviceClass<FEOrder, FEOrderElectro>::init()
  {
    ...
    // vectorTools::computeNormalCellToMacroCellMap(
    ...
    // dftPtr->constraintsNoneDataInfo.initializeScaledConstraints(
    //   d_invSqrtMassVector, dftPtr->constraintsNone);
      ...
      if (cellPtr->is_locally_owned())
        {
          ...
          for (unsigned int q = 0; q < numberQuadraturePoints; ++q)
            {
              d_vEff[iElemCount * numberQuadraturePoints + q] = 0.1;
              ...
              /*
                d_vEffJxWMF[d_normalCellIdToMacroCellIdMap[iElemCount]
                 * numberQuadraturePoints + q] = 0.1
                 // (tempPhi[q] + exchangePotentialVal[q] +
              corrPotentialVal[q]);
                              *
                 d_cellJxWValues[d_normalCellIdToMacroCellIdMap[iElemCount] *
                                                  numberQuadraturePoints +
                                                q];
              //*/
            }
      ...
      const unsigned int externalPotCorrQuadratureId)
  {
    ...
    d_vEffExternalPotCorrJxW.resize(numberPhysicalCells *
                                    numberQuadraturePoints);
    d_vEffExternalPotCorr.resize(numberPhysicalCells * numberQuadraturePoints);
    // d_vEffExternalPotCorrJxWMF.resize(numberPhysicalCells *
    //                                   numberQuadraturePoints);
          ...
          for (unsigned int q = 0; q < numberQuadraturePoints; ++q)
            {
              ...
              // temp[q]
              // 0.1 * feValues.JxW(q);
              d_vEffExternalPotCorr[iElem * numberQuadraturePoints + q] = 0.0;
              /*
              d_vEffExternalPotCorrJxWMF[d_normalCellIdToMacroCellIdMap[iElem] *
                                           numberQuadraturePoints +
                                         q] =
                // temp[q]
                0.1 * feValues.JxW(q);
  ...
  /* CellMatrix distributedDeviceVec */
  template <unsigned int FEOrder, unsigned int FEOrderElectro>
  void
  kohnShamDFTOperatorDeviceClass<FEOrder, FEOrderElectro>::HX(
    distributedDeviceVec<dataTypes::number> &src,
    ...
    const bool onlyHPrimePartForFirstOrderDensityMatResponse)
  {
    ...
    // dftPtr->constraintsNoneDataInfoDevice.set_zero(src, numberWaveFunctions);
    dst.accumulateAddLocallyOwned();
  ...
  /* CellMatrix dealiiVec */
  template <unsigned int FEOrder, unsigned int FEOrderElectro>
  void
  kohnShamDFTOperatorDeviceClass<FEOrder, FEOrderElectro>::HX(
    dealiiVec<dataTypes::number> &           src,
    distributedDeviceVec<dataTypes::number> &projectorKetTimesVector,
    const unsigned int                       localVectorSize,
    const unsigned int                       numberWaveFunctions,
    const bool                               scaleFlag,
    const double                             scalar,
    dealiiVec<dataTypes::number> &           dst,
    const bool                               doUnscalingSrc,
    const bool onlyHPrimePartForFirstOrderDensityMatResponse)
  {
    const unsigned int n_ghosts =
  ...
  kohnShamDFTOperatorDeviceClass<FEOrder,
                                 FEOrderElectro>::getMFBatchedPartitioner()
  {
    return d_matrixFreeBasePtr->getPartitioner();
  }

  template <unsigned int FEOrder, unsigned int FEOrderElectro>
  void
  kohnShamDFTOperatorDeviceClass<FEOrder, FEOrderElectro>::setupMF(
    const unsigned int &numberWaveFunctions)
  {
    if (isMFSetup)
      return;
    ...
    auto shapeInfo =
      dftPtr->matrix_free_data.get_shape_info(dftPtr->d_densityDofHandlerIndex,
                                              dftPtr->d_densityQuadratureId);
    auto shapeData = shapeInfo.get_shape_data();

    auto d_ndofsPerDim       = shapeData.fe_degree + 1;
    auto d_nQuadPointsPerDim = shapeData.n_q_points_1d;
    auto d_ndofsPerCell      = d_ndofsPerDim * d_ndofsPerDim * d_ndofsPerDim;
    auto d_nQuadPointsPerCell =
      d_nQuadPointsPerDim * d_nQuadPointsPerDim * d_nQuadPointsPerDim;

    d_matrixFreeBasePtr =
      std::make_unique<matrixFree<FEOrder + 1, FEOrder + 1, 4>>(
        mpi_communicator, numberWaveFunctions);

    d_matrixFreeBasePtr->reinit(&(dftPtr->matrix_free_data),
                                &(dftPtr->constraintsNone),
                                d_invSqrtMassVector,
                                dftPtr->d_densityDofHandlerIndex,
                                dftPtr->d_densityQuadratureId);

    std::vector<double> vEffMF(d_numLocallyOwnedCells * d_nQuadPointsPerCell,
                               0.0);

    for (auto i = 0; i < vEffMF.size(); i++)
      vEffMF[i] = d_vEff[i]; // + d_vEffExternalPotCorr[i];

    d_matrixFreeBasePtr->setCoeffs(vEffMF);
  }
  ...
  namespace
  {
    ...
    __global__ void
    blockScaleKernel(ValueType *            x,
                     const ValueType *      s,
                     const ValueType        a,
                     const dftfe::size_type nDofs)
    {
      int dofIdx = threadIdx.y + blockIdx.x * blockDim.y;
      int idx =
        threadIdx.x + dofIdx * blockDim.x + blockIdx.y * blockDim.x * nDofs;
      ...
      if (dofIdx < nDofs)
        x[idx] = a * x[idx] * s[dofIdx];
    }
  } // namespace


  /* MatrixFree distributedDeviceVec */
  template <unsigned int FEOrder, unsigned int FEOrderElectro>
  void
  kohnShamDFTOperatorDeviceClass<FEOrder, FEOrderElectro>::HXMF(
    distributedDeviceVec<dataTypes::number> &src,
    distributedDeviceVec<dataTypes::number> &projectorKetTimesVector,
    const unsigned int                       localVectorSize,
    const unsigned int                       numberWaveFunctions,
    ...
    const bool onlyHPrimePartForFirstOrderDensityMatResponse)
  {
    ...
    const int     batch     = numberWaveFunctions / batchSize;

    dim3 threads(batchSize, dftfe::utils::DEVICE_BLOCK_SIZE / batchSize);

    dim3 blocks((batchSize * localVectorSize + dftfe::utils::DEVICE_BLOCK_SIZE -
                 1) /
                  dftfe::utils::DEVICE_BLOCK_SIZE,
                batch);

    int vectorSize = localVectorSize * numberWaveFunctions;
    ...
    blockScaleKernel<<<blocks, threads>>>(
      ...
      dftfe::utils::makeDataTypeDeviceCompatible(
        d_invSqrtMassVectorDevice.begin()),
      dftfe::utils::makeDataTypeDeviceCompatible(scalar),
      dftfe::utils::makeDataTypeDeviceCompatible(localVectorSize));
    ...
    if (scaleFlag)
      {
        blockScaleKernel<<<blocks, threads>>>(
          ...
          dftfe::utils::makeDataTypeDeviceCompatible(
            d_sqrtMassVectorDevice.begin()),
          dftfe::utils::makeDataTypeDeviceCompatible(1.0),
          dftfe::utils::makeDataTypeDeviceCompatible(localVectorSize));
      } //*/
    ...
    d_matrixFreeBasePtr->distribute(src.begin(), numberWaveFunctions);

    /*d_matrixFreeBasePtr->getConstraintsInfo().distribute(src.begin(),
                                                         batchSize,
                                                         numberWaveFunctions);
       //*/

    computeLocalHamiltonianTimesXMF(
      src.begin(),
      numberWaveFunctions,
      dst.begin(),
      onlyHPrimePartForFirstOrderDensityMatResponse); //*/
    ...
    d_matrixFreeBasePtr->distributeSlaveToMaster(dst.begin(),
                                                 src.begin(),
                                                 numberWaveFunctions); //*/

    /*d_matrixFreeBasePtr->getConstraintsInfo().distribute_slave_to_master(
      dst.begin(), batchSize, numberWaveFunctions); //*/
    ...
    if (doUnscalingSrc)
      blockScaleKernel<<<blocks, threads>>>(
        ...
        dftfe::utils::makeDataTypeDeviceCompatible(
          d_sqrtMassVectorDevice.begin()),
        dftfe::utils::makeDataTypeDeviceCompatible(1.0 / scalar),
        dftfe::utils::makeDataTypeDeviceCompatible(localVectorSize)); //*/
  }


  /* MatrixFree dealiiVec */
  template <unsigned int FEOrder, unsigned int FEOrderElectro>
  void
  kohnShamDFTOperatorDeviceClass<FEOrder, FEOrderElectro>::HXMF(
    dealiiVec<dataTypes::number> &           src,
    ...
    const bool onlyHPrimePartForFirstOrderDensityMatResponse)
  {
    ...
    d_matrixFreeBasePtr->distribute(dftfe::utils::makeDataTypeDeviceCompatible(
                                      src.begin()),
                                    numberWaveFunctions); //*/
    ...
    d_matrixFreeBasePtr->distributeSlaveToMaster(
      dftfe::utils::makeDataTypeDeviceCompatible(dst.begin()),
      dftfe::utils::makeDataTypeDeviceCompatible(src.begin()),
      numberWaveFunctions); //*/

    /*d_matrixFreeBasePtr->getConstraintsInfo().distribute_slave_to_master(
      dst.begin(), batchSize, numberWaveFunctions, totalDofSize); //*/
  ...
  kohnShamDFTOperatorDeviceClass<FEOrder, FEOrderElectro>::HXCheby(
    dealiiVec<dataTypes::number> &           src,
    dealiiVec<dataTypes::numberFP32> &       tempFloatArray,
    distributedDeviceVec<dataTypes::number> &projectorKetTimesVector,
    const unsigned int                       localVectorSize,
    const unsigned int                       numberWaveFunctions,
    dealiiVec<dataTypes::number> &           dst,
  ...
  template <unsigned int FEOrder, unsigned int FEOrderElectro>
  void
  kohnShamDFTOperatorDeviceClass<FEOrder, FEOrderElectro>::HXChebyMF(
    distributedDeviceVec<dataTypes::number> &    src,
    distributedDeviceVec<dataTypes::numberFP32> &tempFloatArray,
    distributedDeviceVec<dataTypes::number> &    projectorKetTimesVector,
    const unsigned int                           localVectorSize,
    ...
    bool                                         computePart2)
  {
    ...
    if (!computePart2)
      computeLocalHamiltonianTimesXMF(src.begin(),
                                      numberWaveFunctions,
                                      dst.begin());
    ...
    if (chebMixedPrec)
      {
        ...
        // copy locally owned processor boundary nodes only to dst vector
        copyFloatArrToDoubleArrLocallyOwnedMF<<<blocks, threads>>>(
          dftfe::utils::makeDataTypeDeviceCompatible(tempFloatArray.begin()),
          d_locallyOwnedProcBoundaryNodesVectorDevice.begin(),
          dftfe::utils::makeDataTypeDeviceCompatible(dst.begin()),
          localSize);
