    ...
    if (d_dftParamsPtr->mixingMethod == "ANDERSON_WITH_KERKER")
      {
        ...
        if (false and d_dftParamsPtr->useDevice and
        ...
        if (d_dftParamsPtr->useDevice and
...
#endif
          {
            ...
            CGSolverDevice.solve(
              ...
              kohnShamDFTEigenOperatorDevice.getDeviceBlasHandle(),
              kohnShamDFTEigenOperatorDevice.getShapeFunctionGradientIntegral(),
              d_dftParamsPtr->verbosity);
#endif
          }
        else
        ...
        if (d_dftParamsPtr->spinPolarized == 1)
          {
            ...
            if (!scfConverged &&
                 ...
                 d_dftParamsPtr->allowMultipleFilteringPassesAfterFirstScf))
              {
                ...
                while (maxRes > filterPassTol &&
                       count < 100) // Hardcode for cheby pass, 3 or 4
                  {
                    for (unsigned int s = 0; s < 2; ++s)
                      {
