...
{
  const unsigned int nveccut = d_dftParamsPtr->dc_d3cutoff2;
  ...
  for (int kPoint = 0; kPoint < eigenValuesAllkPoints.size(); ++kPoint)
    {
      ...
      if (residualNormWaveFunctionsAllkPoints[kPoint][nveccut] >
          maxHighestOccupiedStateResNorm)
        {
          maxHighestOccupiedStateResNorm =
            residualNormWaveFunctionsAllkPoints[kPoint][nveccut];
        }
    }
  maxHighestOccupiedStateResNorm =
