...
namespace dftfe
{
  namespace dftUtils
  {
    ...
    public:
      ...
      template <typename NumberType>
      void
      distribute(dealiiVec<NumberType> &fieldVector,
                 const unsigned int     blockSize) const;

      void
      distribute(double *   fieldVector,
                 const int &batchSize,
                 const int &blockSize,
                 const int &totalDofSize) const;

      void
      distributeOpt(distributedDeviceVec<double> &fieldVector,
                    const int &                   batchSize,
                    const int &                   blockSize) const;

      void
      distribute_slave_to_master(double *   fieldVector,
                                 const int &batchSize,
                                 const int &blockSize,
                                 const int &totalDofSize) const;
      ...
      void
      distribute_slave_to_master(dealiiVec<double> &fieldVector,
                                 const unsigned int blockSize) const;
