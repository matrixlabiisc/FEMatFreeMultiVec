...
namespace dftfe
{
  namespace vectorTools
  {
      ...
      const bool                              periodicZ)
    {
      ...
      // dofHandlerPar.get_fe().dofs_per_cell;
      ...
      distributedCPUVec<T> &flattenedArray)
    {
      ...
      globalStartIndices[dealii::Utilities::MPI::this_mpi_process(
        mpi_communicator)] = partitioner->local_to_global(0);
    ...
    void
    computeNormalCellToMacroCellMap(
      const dealii::MatrixFree<3, double> &matrix_free_data,
      const unsigned int                   mfDofHandlerIndex,
      std::vector<unsigned int> &          normalCellIdToMacroCellIdMap)
    {
      ...
      std::map<unsigned int, unsigned int>                 cellGlobalIndices;
          ...
          for (unsigned int iCell = 0; iCell < n_sub_cells; ++iCell)
            {
              cellPtr = matrix_free_data.get_cell_iterator(iMacroCell,
                                                           iCell,
                                                           mfDofHandlerIndex);
              cellGlobalIndices[cellPtr->active_cell_index()] = iElem;
              ++iElem;
            } // subcell loop
        }     // macrocell loop
      ...
      for (; cell != endc; ++cell)
        {
          if (cell->is_locally_owned())
            {
              normalCellIdToMacroCellIdMap[iElemCount] =
                cellGlobalIndices[cell->active_cell_index()];
              ++iElemCount;
            }
        }
    }
